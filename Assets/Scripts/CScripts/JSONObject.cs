...
public float f {
    ...
    {    //create a new JSONObject from a string (this will also create any children, and parse the whole string)
        if (str != null)
        {
            ...
            if (strict)
            {
                ...
                {
                    type = Type.NULL;
                    Log.Warn("Improper (strict) JSON formatting.  First character must be [ or {");
                    return;
                }
            }
            if (str.Length > 0)
                ...
                else
                {
                    ...
                    catch (System.FormatException)
                    {
                        ...
                        switch (str[offset])
                        {
                            ...
                            default:
                                type = Type.NULL;
                                Log.Warn("improper JSON formatting:" + str);
                                return;
                        }
                        string propName = "";
                        ...
                        while (++offset < str.Length)
                        {
                            ...
                            if (str[offset] == '\"')
                            {
                                if (openQuote)
                                {
                                    if (!inProp && depth == 0 && type == Type.OBJECT) {
                                        ...
                                        //Log.Info("PropName : {0}, {1}, {2}, {3}", propName, token_tmp, offset, str);
                                    }
                                    openQuote = false;
                                }
                                else
                                {
                                    if (!inProp && depth == 0 && type == Type.OBJECT) {
                                        //Log.Info("  open : {0}, {1}, n:{2} , s:{3} ", token_tmp, offset, propName, str);
                                        token_tmp = offset;
                                    }
                                    openQuote = true;
                                }
                                if (inProp && str[offset-1] == '\\') {
                                    openQuote = true;
                                }
                            }
                            if (openQuote)
                            ...
                            if (type == Type.OBJECT && depth == 0)
                            {
                                if (str[offset] == ':')
                                {
                                    //Log.Info("  meet : {0}, {1}, n:{2} ", token_tmp, offset, propName);
                                    token_tmp = offset + 1;
                                    inProp = true;
                                }
                            }
                            ...
                            if ((str[offset] == ',' && depth == 0) || depth < 0)
                            {
                                ...
                                //Log.Info("  inner : {0}, {1}, n:{2}, in:{3}", token_tmp, offset, propName, inner);
                                if (inner.Length > 0)
                                {
                                    if (type == Type.OBJECT)
                                        keys.Add(propName);
...
#if !USEFLOAT
    public void Add(double val) { Add(new JSONObject(val)); }
#endif
    ...
    public void Add(string name, bool val) { Add(name, new JSONObject(val)); }
    public void Add(string name, float val) { Add(name, new JSONObject(val)); }
    public void Add(string name, int val) { Add(name, new JSONObject(val)); }
    public void Add(string name, AddJSONConents content) { Add(name, new JSONObject(content)); }
    public void Add(string name, string val) { Add(name, StringObject(val)); }
        ...
        if (type == Type.OBJECT)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                switch (val.type)
                {
                    ...
                    default: Log.Warn("Omitting object: " + (string)keys[i] + " in dictionary conversion"); break;
                }
            }
            return result;
        }
        else Log.Warn("Tried to turn non-Object JSONObject into a dictionary");
        return null;
    }
